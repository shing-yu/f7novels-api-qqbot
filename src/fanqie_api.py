...
# 定义正常模式用来下载番茄小说的函数
def download(url: str, encoding: str, config: dict, _) -> tuple:
    title = None
    last_cid = None
    ...
    try:
        ...
        try:
            ...
            logger.success(f"小说《{title}》已上传到COS")

            logger.success(f"下载完成提示已发送")
...
def update(url: str, encoding: str, start_id: str, file_path: str, config: dict, _) -> tuple:
